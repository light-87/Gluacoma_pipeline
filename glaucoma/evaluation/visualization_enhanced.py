...
Implements advanced visualization techniques for model evaluation and interpretation.
"""
...
class VisualizationManager:
    """Manager for generating advanced visualizations of model results."""
    ...
    def __init__(
        ...
        if create_subdirs:
            ...
            self.feature_dir = self.viz_dir / "features"
    ...
    def plot_cdr_distribution(
        ...
        labels: List[int],
        bins: int = 30,
        output_filename: str = 'cdr_distribution.png'
    ) -> Path:
        """Plot distribution of cup-to-disc ratios.
        
        Args:
            cdrs: List of cup-to-disc ratios
            labels: List of labels (0 for normal, 1 for glaucoma)
        ...
        # Create separate dataframes for normal and glaucoma
        normal_cdrs = cdrs[labels == 0]
        glaucoma_cdrs = cdrs[labels == 1]
        
        # Plot histograms
        if len(normal_cdrs) > 0:
            plt.hist(normal_cdrs, bins=bins, alpha=0.7, label='Normal', color='green')
        
        if len(glaucoma_cdrs) > 0:
            plt.hist(glaucoma_cdrs, bins=bins, alpha=0.7, label='Glaucoma', color='red')
        
        plt.xlabel('Cup-to-Disc Ratio (CDR)', fontsize=12)
        plt.ylabel('Count', fontsize=12)
        plt.title('Cup-to-Disc Ratio Distribution', fontsize=14)
        plt.grid(True, alpha=0.3)
        plt.legend(fontsize=12)
        
        # Add summary statistics as text
        if len(cdrs) > 0:
            stats_text = f"Overall: Mean={np.mean(cdrs):.3f}, Median={np.median(cdrs):.3f}, SD={np.std(cdrs):.3f}"
            plt.figtext(0.5, 0.01, stats_text, ha="center", fontsize=10)
        
        # Save figure
        output_path = self._prepare_save_path(self.distribution_dir, output_filename)
        plt.tight_layout()
        plt.savefig(output_path, dpi=self.dpi, bbox_inches='tight')
        plt.close()
        
        # Store path
        self.visualization_paths['distributions'].append(output_path)
    ...
    def plot_feature_maps(
        ...
        output_filename: str = 'feature_maps.png',
        max_features: int = 16,
        ...
        Args:
            feature_maps: Feature maps array (n_features, height, width)
            output_filename: Name of output file
        ...
        for i in range(grid_size * grid_size):
            ...
            if i < n_features:
                ...
                # Plot feature map
                ax.imshow(feature, cmap=colormap)
        ...
        # Save figure
        output_path = self._prepare_save_path(self.feature_dir, output_filename)
        plt.tight_layout()
        plt.savefig(output_path, dpi=self.dpi, bbox_inches='tight')
        plt.close(fig)
        
        # Store path
        self.visualization_paths['features'].append(output_path)
    ...
    def plot_grad_cam(
        ...
        image: np.ndarray,
        heatmap: np.ndarray,
        output_filename: str = 'grad_cam.png'
    ) -> Path:
        ...
        # Create overlay
        overlay = heatmap_rgb * 0.4 + image * 0.6
        overlay = np.clip(overlay, 0, 1)
    ...
    def plot_metrics_over_epochs(
        ...
        Args:
            metrics_dict: Dictionary mapping metric names to lists of values over epochs
            output_filename: Name of output file
    ...
    def generate_report(
        ...
        for name, value in sorted(metrics.items()):
            if isinstance(value, (int, float)) and not isinstance(value, bool) and not name.startswith('confusion_matrix'):
